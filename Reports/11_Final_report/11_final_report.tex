...
V nadaljevanju članka, je so navedena in opisana povezana dela in že odstoječe aplikacije, ki obstajajo na tem področju. Opisan je postopek priprave podatkov in uporabljene metode strojnega učenja. V drugem delu članka pa je predstavljen eksperiment in rezultati našega dela.
...
\begin{enumerate}
\item{Enostavna za razlago in interpretacijo, lahko jih tudi vizualiziramo}
...
Glavne spabosti odločitvenih dreves so\cite{SciTree}:
\begin{enumerate}
\item{Odločitvena drevesa se lahko preveč prilagodijo učni množici, kar lahko privede do modela, ki ni dovolj splošen. To lahko rešimo z omejitvami vejitev in globine.}
\item{Majhne spremembe v podatkih lahko povzročijo velike spremembe v modelu.}
\item{Problem učenja odločitvenega drevesa je zelo kompleksen, zato se uporabljajo hevristični pristopi, ki ne zagotavljajo optimalne rešitve. Ta problem lahko omilimo 
z uporabo ansambelskih metod.}
\item{Algoritmi za grajenje dreves lahko ustvarijo pristranske modele, če velika razlika v frekvenci pojavitve razredov.}
\end{enumerate}
